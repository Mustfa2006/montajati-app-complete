...
class OrderDetailsService {
  static const String baseUrl = 'https://montajati-app-production.up.railway.app';

  /// 📥 جلب تفاصيل طلب محدد من Backend
  static Future<Order> fetchOrderDetails(String orderId) async {
    try {
      debugPrint('📥 جلب تفاصيل الطلب من Backend: $orderId');
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['success'] == true && jsonData['data'] != null) {
          ...
          debugPrint('✅ تم جلب الطلب بنجاح: ${orderData['id']}');
          debugPrint('📋 نوع الطلب: ${isScheduledOrder ? 'مجدول' : 'عادي'}');

          // ✅ تحويل عناصر الطلب (حسب نوع الطلب)
          final itemsKey = isScheduledOrder ? 'scheduled_order_items' : 'order_items';
          final orderItems = (orderData[itemsKey] as List?)
              ?.map((item) => order_item_model.OrderItem(
                    id: item['id']?.toString() ?? '',
          ...
          final totalAmount = int.tryParse(
              ...
              orderData['total']?.toString() ?? '0') ?? 0;
          final subtotalAmount = int.tryParse(
              orderData['total_amount']?.toString() ?? 
              orderData['subtotal']?.toString() ?? '0') ?? 0;
          final profitAmount = int.tryParse(
              orderData['profit_amount']?.toString() ?? 
              orderData['total_profit']?.toString() ?? '0') ?? 0;
  ...
  static Future<bool> checkBackendConnection() async {
    try {
      ...
      debugPrint('⚠️ Backend غير متاح: $e');
      return false;
    }
  }
